...
module.exports = ({
  ...
  createModuleFromArgs(delivery, { basedir })
    .then(d => {
      ...
      const sentPathname = url.parse(sentRedirect).pathname
      const failurePathname = url.parse(failureRedirect).pathname
      if (failureRedirect) router.get(failurePathname, makePublic)
      if (sentPathname !== failurePathname) {
        router.get(sentPathname, makePublic)
      }
